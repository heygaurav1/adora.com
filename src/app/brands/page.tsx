    ...
    <div className="min-h-screen bg-white text-black font-sans selection:bg-black selection:text-white flex flex-col items-center">
      ...
      <main className="w-full max-w-[1400px] px-4 md:px-8 py-20 flex flex-col items-center">
        ...
        <div className="w-full grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-8">
           {brands.map(brand => (
             <div key={brand} className="h-32 border border-gray-100 flex items-center justify-center hover:border-black transition-all cursor-pointer">
                <span className="text-[14px] font-black uppercase tracking-widest">{brand}</span>
             </div>
           ))}
        </div>
      </main>
